...
// TODO
						...
						getTabbedPane();
						while (message != REQclose) {
							message = new Integer(new String(receiveQuery()));
							
							if (message == REQclose) {
								// we close the current session
								in.close();
								out.close();
							} else if (message == REQserviceCertificate) {
								// automatically gets here first, so can ignore, the user needs to select something
	...
	private byte[] receiveQuery() throws Exception  {		
